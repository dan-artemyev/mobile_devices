...
def billing(input_file):
    ...
    cost = price * traffic
    print("Ip 217.15.20.194 used {0}Mb (or {2}MB - see difference between Mb and MB) and have to pay {1} rubles."
          .format(traffic, cost, traffic / 8))

    dots_list_sorted = sorted(dots_list, key=lambda time: time[0])
    bytes_list = []
    time_list = []
    for dot in dots_list_sorted:
        bytes_list.append(dot[1] * 8 / (2**20))
